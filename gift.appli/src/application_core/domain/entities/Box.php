...
class Box extends Eloq\Model {
      ...
      public function validate(): bool {
            if ($this->statut !== 1) throw new DataErrorException("La box doit être en état créée");
            if ($this->prestation->count() < 2) throw new DataErrorException("La box doit contenir au moins 2 prestations");
      ...
      public function canBeModified(): bool {
            if ($this->statut !== 1) throw new DataErrorException("Impossible de modifier une box non créée");
      ...
      public function preparePrestationsForSync(int $presta_id, int $quantite): array {
            ...
            foreach ($this->prestation as $p) $prestations[$p->id] = ['quantite' => $p->pivot->quantite];
      ...
      public function recalculateMontant(): float {
            ...
            foreach ($this->prestation as $p) $total += $p->tarif * $p->pivot->quantite;
      ...
      public function deliver(string $token): bool {
            if ($this->statut !== 2) throw new DataErrorException("La box doit être validée avant livraison");
